    ...
    function renderBranches(data) {
        ...
        tbody.innerHTML = data.map((branch) => `
            <tr>
                ...
                <td>${branch.region || ""}</td>
                <td><span class="branch-revenue">${number(branch.revenue)}đ</span></td>
                ...
                <td>
                    <div class="manager-name">${branch.manager_name || "Chưa gán"}</div>
                    <div class="manager-phone">${branch.address || ""}</div>
                </td>
                <td><span class="status-badge ${branch.status === "active" ? "status-active" : "status-maintenance"}">${branch.status === "active" ? "Hoạt động" : "Tạm ngừng"}</span></td>
