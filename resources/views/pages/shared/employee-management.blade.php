@extends($employeePage['layout'] ?? 'layouts.manager')

@php
    $page = array_merge([
        'context' => 'manager',
        'title' => 'Quản lý nhân viên',
        'description' => '',
        'createButtonLabel' => 'Thêm nhân viên',
        'createTitle' => 'Thêm nhân viên',
        'createSubtitle' => '',
        'createSubmitLabel' => 'Tạo nhân viên',
        'editTitle' => 'Sửa thông tin nhân viên',
        'editSubtitle' => '',
        'resignTitle' => 'Cho nhân viên nghỉ việc',
        'resignText' => 'Bạn có chắc muốn cho nhân viên này nghỉ việc không?',
        'resignSubmitLabel' => 'Xác nhận nghỉ việc',
        'createSuccessMessage' => 'Thêm nhân viên thành công.',
        'editSuccessMessage' => 'Cập nhật thông tin nhân viên thành công.',
        'resignSuccessMessage' => 'Đã cho nhân viên nghỉ việc.',
        'showBranchFilter' => false,
        'showBranchColumn' => true,
        'canChooseBranchOnCreate' => false,
        'canEditBranch' => false,
        'canChoosePositionOnCreate' => false,
        'canEditPosition' => false,
        'createDefaultPosition' => '',
        'positions' => [],
        'stats' => [],
    ], $employeePage ?? []);

    $branches = collect($branches ?? []);
    $positions = collect($page['positions'] ?? []);
    $hasCreateErrors = $errors->any() && old('_employee_form_mode', 'create') === 'create';
    $hasEditErrors = $errors->any() && old('_employee_form_mode') === 'edit';
    $oldEditAction = old('_employee_id')
        ? str_replace('__EMPLOYEE_ID__', old('_employee_id'), $page['updateUrlTemplate'] ?? '#')
        : '#';
@endphp

@section('title', $page['title'])

@push('styles')
    <link rel="stylesheet" href="{{ asset('assets/client/css/manager/employee-management.css') }}?v={{ time() }}">
@endpush

@section('content')
<div
    id="employeeManagementPage"
    class="manager-employee-page"
    data-context="{{ $page['context'] }}"
    data-create-has-errors="{{ $hasCreateErrors ? '1' : '0' }}"
    data-edit-has-errors="{{ $hasEditErrors ? '1' : '0' }}"
    data-old-employee-id="{{ old('_employee_id') }}"
    data-update-url-template="{{ $page['updateUrlTemplate'] ?? '#' }}"
    data-resign-url-template="{{ $page['resignUrlTemplate'] ?? '#' }}"
    data-show-branch-column="{{ $page['showBranchColumn'] ? '1' : '0' }}"
    data-create-success-message="{{ $page['createSuccessMessage'] }}"
    data-edit-success-message="{{ $page['editSuccessMessage'] }}"
    data-resign-success-message="{{ $page['resignSuccessMessage'] }}"
>
    <script type="application/json" id="employeeManagementData">@json($employees ?? [])</script>

    <header class="manager-employee-header">
        <div>
            <p class="manager-employee-eyebrow">Nhân viên</p>
            <h1>{{ $page['title'] }}</h1>
            @if ($page['description'])
                <p>{{ $page['description'] }}</p>
            @endif
        </div>
    </header>

    @if (session('status'))
        <div class="manager-employee-flash manager-employee-flash--success">{{ session('status') }}</div>
    @endif

    <section class="manager-employee-stats-grid" aria-label="Tổng quan nhân viên">
        @foreach (($page['stats'] ?? []) as $stat)
            <x-kpi-card
                :title="$stat['title']"
                value="0"
                :detail="$stat['detail']"
                :value-attributes="[$stat['attribute'] => true]"
            />
        @endforeach
    </section>

    <section class="manager-employee-table-card">
        <div class="manager-employee-table-header">
            <div>
                <h2>Danh sách nhân viên</h2>
                <span data-table-count>0 người</span>
            </div>

            <div class="manager-employee-toolbar" aria-label="Bộ lọc nhân viên">
                <label class="manager-employee-search-box">
                    <span class="manager-employee-search-icon">⌕</span>
                    <input type="text" placeholder="Tìm tên, email, số điện thoại..." data-search-input>
                </label>

                @if ($page['showBranchFilter'])
                    <select class="manager-employee-filter-select" data-filter-branch aria-label="Lọc chi nhánh">
                        <option value="">Tất cả chi nhánh</option>
                        @foreach ($branches as $branch)
                            <option value="{{ $branch['id'] }}">{{ $branch['name'] }}</option>
                        @endforeach
                    </select>
                @endif

                @if ($positions->isNotEmpty())
                    <select class="manager-employee-filter-select" data-filter-position aria-label="Lọc vị trí">
                        <option value="">Tất cả vị trí</option>
                        @foreach ($positions as $position)
                            <option value="{{ $position['value'] }}">{{ $position['label'] }}</option>
                        @endforeach
                    </select>
                @endif

                <button type="button" class="manager-employee-btn-primary" data-open-create-modal>{{ $page['createButtonLabel'] }}</button>
            </div>
        </div>

        <div class="manager-employee-table-wrapper">
            <table class="manager-employee-table">
                <thead>
                    <tr>
                        <th>Nhân viên</th>
                        <th>Liên hệ</th>
                        <th>Vị trí</th>
                        @if ($page['showBranchColumn'])
                            <th>Chi nhánh</th>
                        @endif
                        <th>Lương</th>
                        <th>Ngày vào làm</th>
                        <th>Kinh nghiệm</th>
                        <th>Trạng thái</th>
                        <th class="manager-employee-actions-col">Thao tác</th>
                    </tr>
                </thead>
                <tbody data-employee-table-body></tbody>
            </table>
        </div>

        <div class="manager-employee-empty-state manager-employee-hidden" data-empty-state>
            <div class="manager-employee-empty-icon">∅</div>
            <p>Không tìm thấy nhân viên nào phù hợp.</p>
        </div>
    </section>

    <div class="manager-employee-overlay" data-create-overlay aria-hidden="true">
        <form class="manager-employee-modal" method="POST" action="{{ $page['storeRoute'] ?? '#' }}" data-create-form>
            @csrf
            <input type="hidden" name="_employee_form_mode" value="create">
            @if (! $page['canChoosePositionOnCreate'] && $page['createDefaultPosition'])
                <input type="hidden" name="position" value="{{ $page['createDefaultPosition'] }}">
            @endif

            <div class="manager-employee-modal-head">
                <div>
                    <div class="manager-employee-modal-title">{{ $page['createTitle'] }}</div>
                    @if ($page['createSubtitle'])
                        <div class="manager-employee-modal-sub">{{ $page['createSubtitle'] }}</div>
                    @endif
                </div>
                <button type="button" class="manager-employee-modal-close" data-close-create-modal aria-label="Đóng">×</button>
            </div>

            @if ($hasCreateErrors)
                <div class="manager-employee-form-errors" data-create-errors>
                    @foreach ($errors->all() as $error)
                        <div>{{ $error }}</div>
                    @endforeach
                </div>
            @else
                <div class="manager-employee-form-errors manager-employee-hidden" data-create-errors></div>
            @endif

            <section class="manager-employee-form-section">
                <div class="manager-employee-form-section-title">Thông tin cá nhân</div>
                <div class="manager-employee-form-grid">
                    <label class="manager-employee-form-group manager-employee-form-group--full">
                        <span>Họ và tên <strong>*</strong></span>
                        <input type="text" name="full_name" maxlength="200" value="{{ old('_employee_form_mode', 'create') === 'create' ? old('full_name') : '' }}" placeholder="VD: Trần Minh Khoa">
                    </label>

                    <label class="manager-employee-form-group">
                        <span>Email đăng nhập <strong>*</strong></span>
                        <input type="email" name="email" maxlength="255" value="{{ old('_employee_form_mode', 'create') === 'create' ? old('email') : '' }}" placeholder="staff.branch@pethotel.test">
                    </label>

                    <label class="manager-employee-form-group">
                        <span>Số điện thoại</span>
                        <input type="tel" name="phone" maxlength="20" value="{{ old('_employee_form_mode', 'create') === 'create' ? old('phone') : '' }}" placeholder="0901234567">
                    </label>

                    <label class="manager-employee-form-group">
                        <span>Mật khẩu <strong>*</strong></span>
                        <input type="password" name="password" autocomplete="new-password" placeholder="Tối thiểu 6 ký tự">
                    </label>

                    <label class="manager-employee-form-group">
                        <span>Xác nhận mật khẩu <strong>*</strong></span>
                        <input type="password" name="password_confirmation" autocomplete="new-password" placeholder="Nhập lại mật khẩu">
                    </label>
                </div>
            </section>

            <section class="manager-employee-form-section">
                <div class="manager-employee-form-section-title">Thông tin công việc</div>
                <div class="manager-employee-form-grid">
                    @if ($page['canChooseBranchOnCreate'])
                        <label class="manager-employee-form-group">
                            <span>Chi nhánh phụ trách <strong>*</strong></span>
                            <select name="branch_id">
                                <option value="">Chọn chi nhánh</option>
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch['id'] }}" @selected(old('_employee_form_mode', 'create') === 'create' && (string) old('branch_id') === (string) $branch['id'])>{{ $branch['name'] }}</option>
                                @endforeach
                            </select>
                        </label>
                    @endif

                    @if ($page['canChoosePositionOnCreate'])
                        <label class="manager-employee-form-group">
                            <span>Vị trí <strong>*</strong></span>
                            <select name="position">
                                <option value="">Chọn vị trí</option>
                                @foreach ($positions as $position)
                                    <option value="{{ $position['value'] }}" @selected(old('_employee_form_mode', 'create') === 'create' && old('position') === $position['value'])>{{ $position['label'] }}</option>
                                @endforeach
                            </select>
                        </label>
                    @endif

                    <label class="manager-employee-form-group">
                        <span>Lương</span>
                        <input type="number" name="salary" min="0" max="200000000" step="0.01" value="{{ old('_employee_form_mode', 'create') === 'create' ? old('salary') : '' }}" placeholder="VD: 12000000">
                    </label>

                    <label class="manager-employee-form-group">
                        <span>Ngày vào làm</span>
                        <input type="date" name="hire_date" value="{{ old('_employee_form_mode', 'create') === 'create' ? old('hire_date') : '' }}">
                    </label>

                    <label class="manager-employee-form-group">
                        <span>Ngày sinh</span>
                        <input type="date" name="birthday" value="{{ old('_employee_form_mode', 'create') === 'create' ? old('birthday') : '' }}">
                    </label>

                    <label class="manager-employee-form-group manager-employee-form-group--full">
                        <span>Kinh nghiệm</span>
                        <input type="text" name="experience" maxlength="50" value="{{ old('_employee_form_mode', 'create') === 'create' ? old('experience') : '' }}" placeholder="VD: 2 năm chăm sóc khách hàng">
                    </label>

                    <label class="manager-employee-form-group manager-employee-form-group--full">
                        <span>Ghi chú</span>
                        <textarea name="notes" maxlength="1000" placeholder="Ghi chú thêm về nhân viên...">{{ old('_employee_form_mode', 'create') === 'create' ? old('notes') : '' }}</textarea>
                    </label>
                </div>
            </section>

            <div class="manager-employee-modal-footer">
                <button type="button" class="manager-employee-btn-secondary" data-close-create-modal>Hủy bỏ</button>
                <button type="submit" class="manager-employee-btn-primary">{{ $page['createSubmitLabel'] }}</button>
            </div>
        </form>
    </div>

    <div class="manager-employee-overlay" data-edit-overlay aria-hidden="true">
        <form class="manager-employee-modal" method="POST" action="{{ $oldEditAction }}" data-edit-form>
            @csrf
            @method('PATCH')
            <input type="hidden" name="_employee_form_mode" value="edit">
            <input type="hidden" name="_employee_id" value="{{ old('_employee_id') }}" data-edit-employee-id>

            <div class="manager-employee-modal-head">
                <div>
                    <div class="manager-employee-modal-title">{{ $page['editTitle'] }}</div>
                    @if ($page['editSubtitle'])
                        <div class="manager-employee-modal-sub">{{ $page['editSubtitle'] }}</div>
                    @endif
                </div>
                <button type="button" class="manager-employee-modal-close" data-close-edit-modal aria-label="Đóng">×</button>
            </div>

            @if ($hasEditErrors)
                <div class="manager-employee-form-errors" data-edit-errors>
                    @foreach ($errors->all() as $error)
                        <div>{{ $error }}</div>
                    @endforeach
                </div>
            @else
                <div class="manager-employee-form-errors manager-employee-hidden" data-edit-errors></div>
            @endif

            <section class="manager-employee-form-section">
                <div class="manager-employee-form-section-title">Thông tin có thể chỉnh sửa</div>
                <div class="manager-employee-form-grid">
                    <label class="manager-employee-form-group manager-employee-form-group--full">
                        <span>Họ và tên <strong>*</strong></span>
                        <input type="text" name="full_name" maxlength="200" value="{{ $hasEditErrors ? old('full_name') : '' }}" data-edit-field="name" placeholder="VD: Trần Minh Khoa">
                    </label>

                    <label class="manager-employee-form-group">
                        <span>Số điện thoại</span>
                        <input type="tel" name="phone" maxlength="20" value="{{ $hasEditErrors ? old('phone') : '' }}" data-edit-field="phone" placeholder="0901234567">
                    </label>

                    @if ($page['canEditBranch'])
                        <label class="manager-employee-form-group">
                            <span>Chi nhánh phụ trách <strong>*</strong></span>
                            <select name="branch_id" data-edit-field="branchId">
                                <option value="">Chọn chi nhánh</option>
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch['id'] }}" @selected($hasEditErrors && (string) old('branch_id') === (string) $branch['id'])>{{ $branch['name'] }}</option>
                                @endforeach
                            </select>
                        </label>
                    @endif

                    @if ($page['canEditPosition'])
                        <label class="manager-employee-form-group">
                            <span>Vị trí <strong>*</strong></span>
                            <select name="position" data-edit-field="position">
                                @foreach ($positions as $position)
                                    <option value="{{ $position['value'] }}" @selected($hasEditErrors && old('position') === $position['value'])>{{ $position['label'] }}</option>
                                @endforeach
                            </select>
                        </label>
                    @endif

                    <label class="manager-employee-form-group">
                        <span>Lương</span>
                        <input type="number" name="salary" min="0" max="200000000" step="0.01" value="{{ $hasEditErrors ? old('salary') : '' }}" data-edit-field="salary" placeholder="VD: 12000000">
                    </label>

                    <label class="manager-employee-form-group">
                        <span>Ngày vào làm</span>
                        <input type="date" name="hire_date" value="{{ $hasEditErrors ? old('hire_date') : '' }}" data-edit-field="hireDate">
                    </label>

                    <label class="manager-employee-form-group">
                        <span>Ngày sinh</span>
                        <input type="date" name="birthday" value="{{ $hasEditErrors ? old('birthday') : '' }}" data-edit-field="birthday">
                    </label>

                    <label class="manager-employee-form-group manager-employee-form-group--full">
                        <span>Kinh nghiệm</span>
                        <input type="text" name="experience" maxlength="50" value="{{ $hasEditErrors ? old('experience') : '' }}" data-edit-field="experience" placeholder="VD: 2 năm chăm sóc khách hàng">
                    </label>

                    <label class="manager-employee-form-group manager-employee-form-group--full">
                        <span>Ghi chú</span>
                        <textarea name="notes" maxlength="1000" data-edit-field="notes" placeholder="Ghi chú thêm về nhân viên...">{{ $hasEditErrors ? old('notes') : '' }}</textarea>
                    </label>
                </div>
            </section>

            <div class="manager-employee-modal-footer">
                <button type="button" class="manager-employee-btn-secondary" data-close-edit-modal>Hủy bỏ</button>
                <button type="submit" class="manager-employee-btn-primary">Lưu thay đổi</button>
            </div>
        </form>
    </div>

    <div class="manager-employee-overlay" data-resign-overlay aria-hidden="true">
        <form class="manager-employee-modal manager-employee-confirm-modal" method="POST" action="#" data-resign-form>
            @csrf

            <div class="manager-employee-modal-head">
                <div>
                    <div class="manager-employee-modal-title">{{ $page['resignTitle'] }}</div>
                    <div class="manager-employee-modal-sub">Hành động này không xóa dữ liệu nhân viên.</div>
                </div>
                <button type="button" class="manager-employee-modal-close" data-close-resign-modal aria-label="Đóng">×</button>
            </div>

            <div class="manager-employee-confirm-body">
                <p>{{ $page['resignText'] }}</p>
                <strong data-resign-employee-name></strong>
            </div>

            <div class="manager-employee-modal-footer">
                <button type="button" class="manager-employee-btn-secondary" data-close-resign-modal>Hủy bỏ</button>
                <button type="submit" class="manager-employee-btn-danger">{{ $page['resignSubmitLabel'] }}</button>
            </div>
        </form>
    </div>

    <div class="manager-employee-toast manager-employee-hidden" data-toast role="status" aria-live="polite"></div>
</div>
@endsection

@push('scripts')
    <script src="{{ asset('assets/client/js/manager/employee-management.js') }}?v={{ time() }}"></script>
@endpush
